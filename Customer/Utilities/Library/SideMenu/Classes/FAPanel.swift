...
open class FAPanelController: UIViewController {
    ...
    open func openPandel() {
        let language = LocalDataManager.getSelectedLanguage()
        if language == LanguageSelection.ENGLISH.rawValue {
            self.openLeft(animated: true)
        } else {
            self.openRight(animated: true)
        }
    }
    ...
    var isNavigationComeFromSideMenu : Bool = false
    internal var tempViewContoller: SideMenuVC? = nil
